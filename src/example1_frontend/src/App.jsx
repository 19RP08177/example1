...
import { example1_backend } from 'declarations/example1_backend';
import './index.scss';
...
function App() {
  ...
  const updateIdentity = (identity) => {
    if (identity) {
      ...
      const actor = Actor.createActor(example1_backend, { agent: agent });
      example1_backend.setActor(actor); // Set the actor for example1_backend
    } else {
      setPrincipal(null);
      example1_backend.setActor(null); // Clear the actor
    }
  };
  useEffect(() => {
  ...
  const fetchStudents = async () => {
    try {
      const studentsList = await example1_backend.getStudents();
  ...
  const fetchCourses = async () => {
    try {
      const coursesList = await example1_backend.getCourses();
  ...
  const fetchDepartments = async () => {
    try {
      const departmentsList = await example1_backend.getDepartments();
  ...
  const fetchStudentCourses = async (studentId) => {
    try {
      const studentCoursesList = await example1_backend.getStudentCourses(studentId);
  ...
  const handleAddStudent = async (event) => {
    ...
    try {
      await example1_backend.addStudent(newStudent.firstName, newStudent.lastName, newStudent.school);
  ...
  const handleAddCourse = async (event) => {
    ...
    try {
      await example1_backend.addCourse(newCourse.courseId, newCourse.courseName);
  ...
  const handleAddDepartment = async (event) => {
    ...
    try {
      ...
      await example1_backend.addDepartment(newDepartment.DeptId, newDepartment.departmentName);
  ...
  const handleAssignCourseToStudent = async () => {
    ...
    try {
      await example1_backend.assignCourseToStudent(selectedStudentId, selectedCourseId);
  ...
  return (
    <main>
      ...
      <h1>Students and Courses Management (example1)</h1>
